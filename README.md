...
Since the Pi is headless, I've included an installer script to make the application start on boot as a systemctl enabled service. Just navigate to the repo folder in terminal and type " sh install-as-service.sh". To stop the service: "sudo systemctl stop gauss-yns.service".
